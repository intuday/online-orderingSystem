    ...
    <div className="px-4 py-2">
      <AnimatePresence>
        {unlockedOffers.map((unlocked) => {
          // Compute savings outside JSX — no IIFE in render
          const claimedPromo  = promoItems.find(
            (pi) => pi.offerId === unlocked.offer.id
          );
          const savingsAmount = claimedPromo
            ? (claimedPromo.price - claimedPromo.promoPrice) * claimedPromo.quantity
            : 0;
            ...
            >
              <div className={`flex items-center gap-3 p-3 rounded-2xl border ${
                ...
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                  unlocked.isClaimed ? "bg-green-100" : "bg-orange-100"
                }`}>
                  {unlocked.isClaimed
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {unlocked.isClaimed ? (
                    <p className="text-[10px] text-green-600 font-medium">
                      ✓ Reward claimed! Saving {formatCurrency(savingsAmount)}
                    </p>
                  ) : (
